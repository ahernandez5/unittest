...
{
    cout << "3350 UNIT TEST \n";
    cout << "Aurora Hernandez \n";

    // preprocessor unit testing 
#ifdef UNIT_TEST
    if (argc != 2) {
        printf("tolerance val out of scope/not found, try different value \n)");
        return 1;

    } else {
        int unittest(double tolerance);
        // converting user input char to double
        double toleranceInput = atof(argv[1]);

        int numberOfErrors = unittest(toleranceInput);

        printf("errors found: %i\n", numberOfErrors);
    }

    //conditional else-line
#else
...
// preprocessor directives unit testing
#ifdef UNIT_TEST
// array list (data)
double arrayData [] = {
    ...
    7.9521, -4.6506, 9.2121, 0.8632, -0.5048, -7777
};
...
int unittest(double tolerance)
{
    ...
    int i = 0;
    int row = 1;
    double expectedLength, expectedX, expectedY;

    //Copied the header from the main program run
    printf("     ----------------  --------------------------  \n");
    printf("     known input:      actual output:              \n");
    printf("     X        Y        length    X        Y        \n");
    printf("     -------  -------  --------  -------  -------  \n");
    while (arrayData[i] != -7777) {

        //read into a vector
        //read known x and y into a vector ki
        Vec ki = {arrayData[i], arrayData[i + 1]};
        //read the expected length, x and y
        expectedLength = arrayData[i + 2];
        expectedX = arrayData[i + 3];
        expectedY = arrayData[i + 4];
        i += 5; //increment 5 at a time


        //the result of vecNormalize
        Vec ao = {ki[0], ki[1]};
        Flt ret = vecNormalize(ao);

        //Copied the printf from main to format the columns
        printf("%3i  %7.4lf  %7.4lf  %8.4lf  %7.4lf  %7.4lf",
                row, ki[0], ki[1], ret, ao[0], ao[1]);


        // abs(actual - expected) is the error value
        //if its greater than tolerance, then that value has an error
        double lengthError = abs(ret - expectedLength);
        double xError = abs(ao[0] - expectedX);
        double yError = abs(ao[1] - expectedY);

        //Testing 3 different possible errors
        //first if length is off
